...
| 2026-07-17 | Foreign repos: `upstream` remote = flag, docs skipped | sandbox/expo (a fork kept for upstreaming a PR) was 1435 of 1925 docs — someone else's doctrine drowning every search. The `upstream` remote is the standard fork marker and classified all 30 repos correctly. Rejected: authorship share — probed against the fleet and it misclassifies work repos (cuanto: 34/5157 commits under the personal email; bodas-app: 0/118) where the user commits under another identity or barely at all. Foreign repos stay listed (with the flag, zero docs) so `docs list` explains itself; `LORE_DOCS_EXCLUDE` (comma-separated `/`-bounded path suffixes) is the manual escape hatch that skips repos entirely. |
